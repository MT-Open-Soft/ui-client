...
function LanguageMovies() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const response = await axios.get(`${apiURL}?page=1&pageSize=20&languages=${language}`);
  ...
  const fetchNextPage = async () => {
    try {
      const response = await axios.get(`${apiURL}?page=${currentPage + 1}&pageSize=20&languages=${language}`);

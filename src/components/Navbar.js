    ...
    <nav className="flex items-center justify-between flex-wrap bg-gray-800 py-4 lg:px-12 shadow border-solid border-t-2 border-blue-300">
      ...
      <div className={`menu w-full lg:block flex-grow lg:flex lg:items-center lg:w-auto lg:px-3 px-8 ${isMenuOpen ? 'block' : 'hidden'}`}>
        <div className="text-md font-bold text-white lg:flex-grow">
          <a href="movies" onClick={()=>{
              toggleMenu();
              setIsSearchVisible(true);
            }} className="block mt-4 lg:inline-block lg:mt-0 navbar-link hover:bg-blue-300 hover:text-white px-4 py-2 rounded">Movies</a>
          <a href="series" onClick={()=>{
              toggleMenu();
              setIsSearchVisible(true);
            }} className="block mt-4 lg:inline-block lg:mt-0 navbar-link hover:bg-blue-300 hover:text-white px-4 py-2 rounded">Series</a>
          <a href="pricing" onClick={()=>{
              toggleMenu();
              setIsSearchVisible(true);
            }} className="block mt-4 lg:inline-block lg:mt-0 navbar-link hover:bg-blue-300 hover:text-white px-4 py-2 rounded">Pricing plans</a>
        </div>
        {(path=="") ? <div className={`relative mx-auto text-gray-600 lg:hidden  'block' : 'hidden'}`}>
...
</div> : <div className={`relative mx-auto text-gray-600}`}>
